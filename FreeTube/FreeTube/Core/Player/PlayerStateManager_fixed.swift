...
final class PlayerStateManager {
    ...
    /// Wait until AVPlayerItem is actually ready before starting playback.
    /// `loadState = .readyToPlay` is our UI state; it does not guarantee
    /// that AVFoundation finished preparing the underlying item.
    private func waitUntilReadyAndPlay(_ item: AVPlayerItem) {
        Task { @MainActor [weak self] in
            ...
            guard item.status == .readyToPlay else {
                let error = item.error?.localizedDescription ?? "Unknown AVPlayerItem error"

                self.log.error(
                    "waitUntilReadyAndPlay failed: \(error, privacy: .public)"
                )
